...
public ABMIX2d_initialize_temperature_salinity
...
subroutine ABMIX2d_initialize_topography ( D, G, param_file, max_depth )
  ...
  call get_param(param_file, mod, "ABMIX2D_SHELF_WIDTH", dome2d_width_bay, &
                 'Width of shelf, as fraction of domain, in 2d ABMIX configuration.', &
                 units='nondim',default=0.1)
  call get_param(param_file, mod, "ABMIX2D_BASIN_WIDTH", dome2d_width_bottom, &
                 'Width of deep ocean basin, as fraction of domain, in 2d ABMIX configuration.', &
                 units='nondim',default=0.3)
  call get_param(param_file, mod, "ABMIX2D_SHELF_DEPTH", dome2d_depth_bay, &
                 'Depth of shelf, as fraction of basin depth, in 2d ABMIX configuration.', &
...
!! COMMENT OUT TO REMOVE INITIAL DENSE WATER ON SHELF:
         x = ( G%geoLatT(i,j) - G%south_lat ) / G%len_lat;
         if ( x .le. 0.1 ) then
           h(i,j,1:nz-1) = GV%Angstrom;
           h(i,j,nz) = G%bathyT(i,j) - (nz-1) * GV%Angstrom;
         end if
...
!> Initialize temperature and salinity in the 2d ABMIX configuration
subroutine ABMIX2d_initialize_temperature_salinity ( T, S, h, G, param_file, eqn_of_state)
  type(ocean_grid_type),                     intent(in)  :: G !< Ocean grid structure
...
end subroutine ABMIX2d_initialize_temperature_salinity

!> Set up sponges in 2d ABMIX configuration
subroutine ABMIX2d_initialize_sponges(G, GV, tv, param_file, use_ALE, CSp, ACSp)
  type(ocean_grid_type),   intent(in) :: G !< Ocean grid structure
  type(verticalGrid_type), intent(in) :: GV !< Vertical grid structure
  ...
  call get_param(param_file, mod, "ABMIX2D_WEST_SPONGE_TIME_SCALE", dome2d_west_sponge_time_scale, &
                 'The time-scale on the west edge of the domain for restoring T/S\n' //&
                 'in the sponge. If zero, the western sponge is disabled', &
                 units='s', default=0.)
  call get_param(param_file, mod, "ABMIX2D_EAST_SPONGE_TIME_SCALE", dome2d_east_sponge_time_scale, &
                 'The time-scale on the east edge of the domain for restoring T/S\n' //&
                 'in the sponge. If zero, the eastern sponge is disabled', &
                 units='s', default=0.)
  call get_param(param_file, mod, "ABMIX2D_WEST_SPONGE_WIDTH", dome2d_west_sponge_width, &
                 'The fraction of the domain in which the western sponge for restoring T/S\n' //&
                 'is active.', &
                 units='nondim', default=0.1)
  call get_param(param_file, mod, "ABMIX2D_EAST_SPONGE_WIDTH", dome2d_east_sponge_width, &
  ...
  if (associated(CSp)) call MOM_error(FATAL, &
     "ABMIX2d_initialize_sponges called with an associated control structure.")
  if (associated(ACSp)) call MOM_error(FATAL, &
     "ABMIX2d_initialize_sponges called with an associated ALE-sponge control structure.")
  ...
  do j=js,je ; do i=is,ie
    if (G%mask2dT(i,j) > 0.) then ! Only set damping rate for wet points
      x = ( G%geoLatT(i,j) - G%south_lat ) / G%len_lat ! Non-dimensional position within domain (0,1)
  ...
  if (use_ALE) then
    ...
    do j=js,je ; do i=is,ie
      ...
      do k = nz,1,-1
        ...
        if ( ( G%geoLatT(i,j) - G%south_lat ) / G%len_lat < dome2d_west_sponge_width ) S(i,j,k) = S_ref + S_range
